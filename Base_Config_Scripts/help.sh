...
snap_shot()
{
  # call snap_shot.sh in the Base_Config_Scripts/source/ directroy 
  SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
  SOURCE_DIR="$SCRIPT_DIR/source"
  "$SOURCE_DIR/snap_shot.sh"
}
...
omreport_sh_printout()
{
  ...
  SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
  SOURCE_DIR="$SCRIPT_DIR/source"
  "$SOURCE_DIR/Omreport_Linux.sh" || echo "Failed to load bash script from ~/Base_Config_Scripts/source/"
}
...
display_menu()
{
  ...
  case $choice in
    ...
    8)
      ...
      SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
      SOURCE_DIR="$SCRIPT_DIR/source"
      "$SOURCE_DIR/firewall_restart.sh"  || echo "Failed to load bash script from ~/Base_Config_Scripts/source/"
      ;;
    9) 
      clear
      echo
