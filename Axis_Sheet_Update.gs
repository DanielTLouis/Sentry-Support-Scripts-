...
function piechart_axis_up_down() {
  ...
  data.forEach(row => {
    ...
    const isDate = !isNaN(Date.parse(d));
    ...
    } else if (d === "supported" || d === "orderable") {
      eomCounts["Supported"]++;
    } else if (d === "unsupported") {
      eomCounts["Unsupported"]++;
    } else if (d === "" || d === "unknown" || d === "unknown status") {
      eomCounts["Unknown"]++;
    } else {
  ...
  sheet.getRange(TABLE_TOP_ROW, AXIS_TABLE_COL, 10, 2).clearContent(); // J2:K11
  sheet.getRange(TABLE_TOP_ROW, EOM_TABLE_COL, 10, 2).clearContent();  // M2:N11
  sheet.getRange(TABLE_TOP_ROW, WAR_TABLE_COL, 10, 2).clearContent();  // P2:Q11
